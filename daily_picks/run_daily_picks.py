        ...
        for p in picks:
            ...
            if p["contracts"]:
                for c in p["contracts"]:
                    ...
                    contract_lines += (
                        ...
                        f"<b style='color:#ff5a30;'>${c['strike']:.2f}C</b> "
                        f"exp {esc(p['exp'])} · "
            ...
            <tr style='border-top:1px solid #1a2735;'>
              <td style='padding:10px 12px;vertical-align:top;'>
                <div style='font-weight:700;font-size:15px;color:#ff5a30;letter-spacing:1px;'>{esc(p['ticker'])}</div>
...
<table cellspacing='0' cellpadding='0' style='max-width:680px;margin:0 auto;background:#0a1521;border:1px solid #1a2735;border-radius:8px;width:100%;'>
  <tr>
    <td style='padding:18px 20px;border-bottom:1px solid #1a2735;background:linear-gradient(135deg,#0a1521,#0e1b2c);'>
      <div style='font-family:Orbitron,sans-serif;font-size:18px;letter-spacing:3px;color:#ff5a30;'>RedOptianda — DAILY PICKS</div>
  ...
  <tr>
    <td style='padding:14px 20px;border-top:1px solid #1a2735;font-size:10px;color:#445566;line-height:1.6;'>
      ...
      <a href='https://alexreed122287.github.io/RedOptianda/' style='color:#ff5a30;text-decoration:none;'>alexreed122287.github.io/RedOptianda</a>.
...
def send_email(html_body, n_picks):
    ...
    subject = f"RedOptianda — {n_picks} bullish picks ({subject_date})"
    payload = {
        "from": EMAIL_FROM,
        "to": [EMAIL_TO],
        "subject": subject,
